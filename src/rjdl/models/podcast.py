...
class Podcast:
    ...
    """This object represents a RadioJavan podcast.
    ...
    def __init__(self, url: str):
        try:
            ...
            data = BeautifulSoup(content, "html.parser").findAll(
                "div", href=False, attrs={"id": "tracklist"}
            )
            if data:
                tracklist = (
                    data[0]
                ...
                self.tracklist = list(
                    filter(
                        lambda x: x,
                        [track.strip() for track in tracklist.split("\n")[2:-3]],
                    )
                )
            else:
                self.tracklist = []
            ...
            self.download_link = (
                f"{response.json()['host']}/media/podcast/mp3-192/{file_name}.mp3"
            )
