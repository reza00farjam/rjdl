...
class Music:
    ...
    def __init__(self, url: str, quality: str = "320"):
        try:
            ...
            data = BeautifulSoup(content, "html.parser").findAll(
                "img", href=False, attrs={"class": "cover"}
            )
            self.cover = data[0]["src"]

            data = BeautifulSoup(content, "html.parser").findAll(
                "div", href=False, attrs={"class": "lyricsContainer"}
            )
            ...
            data = BeautifulSoup(content, "html.parser").findAll(
                "div", href=False, attrs={"class": "mp3Description"}
            )
            details = (
                data[0]
                .prettify()
            ...
            response = requests.post(
                "https://www.radiojavan.com/mp3s/mp3_host", params={"id": file_name}
            )
            self.download_link = f"{response.json()['host']}/media/mp3/mp3-\
                {self.quality}/{file_name}.mp3"
